...
class anos_dourados(db.Model):
    ...
    def __init__(self, nome, url_imagem, ano_lancamento, infancia_do, trailer_desenho):
        ...
        self.trailer_desenho = trailer_desenho.replace(
            "watch?v=", "embed/", len(trailer_desenho))
...
def listar_anos_dourados():
    ...
    return render_template('listar-desenhos.html', listaDeDesenhos=desenhos)
...
def create():
    ...
    if request.method == 'POST':
        form = request.form
        desenhos = anos_dourados(form['nome'], form['url_imagem'],
                                 form['ano_lancamento'], form['infancia_do'], form['trailer_desenho'])
        desenhos.save()
...
def update(desenho_id):
    ...
    if request.method == 'POST':
        form = request.form
        new_data = anos_dourados(form['nome'], form['url_imagem'],
                                 form['ano_lancamento'], form['infancia_do'], form['trailer_desenho'])
